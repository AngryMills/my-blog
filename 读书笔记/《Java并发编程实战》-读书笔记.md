...
barrier 类似闭锁，能够阻塞线程，**关卡和闭锁关键不同在于，所有线程必须同时达到关卡点**。闭锁等待的是事件，关卡等待的是其他线程。

### 5.6 为计算结果建立高效、可伸缩的告诉缓存

用HashMap和ConcurrentHashMap(同步安全)做内存存储。
